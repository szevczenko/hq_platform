...
{
    if (!s_pending_init) {
        osal_mutex_create(&s_pending_mutex, "tb_attr");
...
{
    if (s_shared_cb != NULL) {
        s_shared_cb(payload, s_shared_user_data);
    }
}

static int send_kv_attribute(tb_client_t *client, const char *key, cJSON *root)
{
...
int tb_attributes_send_int(tb_client_t *client, const char *key, int64_t value)
{
    ...
    return send_kv_attribute(client, key, root);
}
...
static int subscribe_response_topic(tb_client_t *client)
{
    ...
    int ret = tb_client_subscribe(client, TB_ATTRIBUTE_RESPONSE_SUB,
                                  attr_response_handler, TB_ATTR_REQUEST_TIMEOUT_MS);
    if (ret == 0) {
...
int tb_attributes_request_client(tb_client_t *client, const char *keys[],
                                 ...
                                 void *user_data, uint32_t timeout_ms)
{
    return attr_request_common(client, keys, num_keys, "clientKeys",
                               cb, user_data, timeout_ms);
}
...
int tb_attributes_request_shared(tb_client_t *client, const char *keys[],
                                 ...
                                 void *user_data, uint32_t timeout_ms)
{
    return attr_request_common(client, keys, num_keys, "sharedKeys",
                               cb, user_data, timeout_ms);
}
                            ...
                            void *user_data)
{
    ...
    if (!s_shared_subscribed) {
        int ret = tb_client_subscribe(client, TB_ATTRIBUTE_TOPIC,
                                      shared_attr_handler, TB_ATTR_REQUEST_TIMEOUT_MS);
        if (ret == 0) {
