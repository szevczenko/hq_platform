...
static int fw_report_state(tb_fw_state_t state, const char *error)
{
    ...
    cJSON_AddStringToObject(root, TB_FW_TELEM_CUR_TITLE, s_fw.current_title);
    cJSON_AddStringToObject(root, TB_FW_TELEM_CUR_VERSION, s_fw.current_version);
    cJSON_AddStringToObject(root, TB_FW_TELEM_STATE, fw_state_to_string(state));
...
static int fw_request_chunk(void)
{
    ...
    snprintf(topic, sizeof(topic), TB_FW_REQ_TOPIC_FMT,
             s_fw.request_id, s_fw.next_chunk);

    if (s_fw.chunk_size == 0 || s_fw.chunk_size > s_fw.target_size) {
        payload[0] = '\0';
    } else {
...
static void fw_fail(const char *reason)
{
    ...
    fw_report_state(TB_FW_STATE_FAILED, reason ? reason : "firmware update failed");
}
                             ...
                             size_t payload_len)
{
    ...
    if (osal_ota_write((const uint8_t *)payload, payload_len) != OSAL_SUCCESS) {
    ...
    if (s_fw.downloaded_size >= s_fw.target_size) {
        ...
        if (s_fw.on_applied != NULL) {
            s_fw.on_applied(s_fw.current_title, s_fw.current_version,
                            s_fw.user_data);
        }
        return;
    }
...
static void fw_attributes_cb(const char *response_json, void *user_data)
{
    ...
    cJSON *source = cJSON_IsObject(shared) ? shared : root;

    cJSON *fw_title = cJSON_GetObjectItemCaseSensitive(source, TB_FW_ATTR_TITLE);
    cJSON *fw_version = cJSON_GetObjectItemCaseSensitive(source, TB_FW_ATTR_VERSION);
    cJSON *fw_size = cJSON_GetObjectItemCaseSensitive(source, TB_FW_ATTR_SIZE);
    cJSON *fw_checksum = cJSON_GetObjectItemCaseSensitive(source, TB_FW_ATTR_CHECKSUM);
    cJSON *fw_checksum_alg = cJSON_GetObjectItemCaseSensitive(source, TB_FW_ATTR_CHECKSUM_ALG);
    ...
    if (cJSON_IsString(fw_checksum_alg) && fw_checksum_alg->valuestring != NULL) {
...
int tb_firmware_update_request_check(tb_client_t *client)
{
    ...
    return tb_attributes_request_shared(client,
                                        shared_keys,
...
void tb_firmware_update_deinit(tb_client_t *client)
{
    ...
    if (s_fw.subscribed) {
        tb_client_unsubscribe(client, TB_FW_RESP_TOPIC_SUB, TB_FW_TIMEOUT_MS);
    }
