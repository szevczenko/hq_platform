                                       ...
                                       size_t payload_len)
{
    if (s_provision_cb != NULL) {
        s_provision_cb(payload, s_provision_user_data);
        s_provision_cb = NULL;
                         ...
                         uint32_t timeout_ms)
{
    ...
    if (req->provision_device_key == NULL || req->provision_device_secret == NULL) {
        return -1;
    }

    /* Subscribe to response topic */
    if (!s_provision_subscribed) {
        int ret = tb_client_subscribe(client, TB_PROVISION_RESPONSE_TOPIC,
    ...
    cJSON_AddStringToObject(root, "provisionDeviceKey", req->provision_device_key);
    cJSON_AddStringToObject(root, "provisionDeviceSecret", req->provision_device_secret);

    if (req->credentials_type != NULL && req->credentials_type[0] != '\0') {
        cJSON_AddStringToObject(root, "credentialsType", req->credentials_type);
    }
