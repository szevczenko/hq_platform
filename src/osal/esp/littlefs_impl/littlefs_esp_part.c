...
int littlefs_esp_part_read_mmap(const struct lfs_config *c, lfs_block_t block,
    ...
    if (part_off > efs->partition->size || part_off + size > efs->partition->size) {
        osal_log_error("attempt to read out bounds of mmaped region %08x-%08x", (unsigned int)part_off, (unsigned int)(part_off + size));
        return LFS_ERR_IO;
    }
...
int littlefs_esp_part_read(const struct lfs_config *c, lfs_block_t block,
    ...
    if (err) {
        osal_log_error("failed to read addr %08x, size %08x, err %d", (unsigned int) part_off, (unsigned int) size, err);
        return LFS_ERR_IO;
    }
    return 0;
}
...
int littlefs_esp_part_write(const struct lfs_config *c, lfs_block_t block,
    ...
    if (err) {
        osal_log_error("failed to write addr %08x, size %08x, err %d", (unsigned int) part_off, (unsigned int) size, err);
        return LFS_ERR_IO;
    }
    return 0;
}
...
int littlefs_esp_part_erase(const struct lfs_config *c, lfs_block_t block) {
    ...
    if (err) {
        osal_log_error("failed to erase addr %08x, size %08x, err %d", (unsigned int) part_off, (unsigned int) c->block_size, err);
        return LFS_ERR_IO;
    }
    return 0;
