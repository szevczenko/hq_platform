...
#define OSAL_OTA_STATE_PARTITION      "storage"
#define OSAL_OTA_STATE_MOUNT_POINT    "/littlefs"
...
static osal_status_t osal_ota_state_ensure_storage(void)
{
    ...
    osal_status_t status = osal_initfs(NULL, OSAL_OTA_STATE_PARTITION,
                                       OSAL_OTA_STATE_MOUNT_POINT, 0U, 0U);
    if (status != OSAL_SUCCESS) {
        return status;
    }

    status = osal_mount(OSAL_OTA_STATE_PARTITION, OSAL_OTA_STATE_MOUNT_POINT);
    if (status == OSAL_SUCCESS) {
        return OSAL_SUCCESS;
    }

    status = osal_mkfs(NULL, OSAL_OTA_STATE_PARTITION,
                       OSAL_OTA_STATE_MOUNT_POINT, 0U, 0U);
    if (status != OSAL_SUCCESS) {
        return status;
    }

    return osal_mount(OSAL_OTA_STATE_PARTITION, OSAL_OTA_STATE_MOUNT_POINT);
}
